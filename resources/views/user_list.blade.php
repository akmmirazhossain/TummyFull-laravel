...
<table class="table table-zebra">
    ...
    <tbody>
        ...
        <tr onclick="window.location='{{ route('user.show', $user->mrd_user_id) }}'" style="cursor: pointer;" class="hover">
            <td>{{ $user->mrd_user_id }}</td>
            <td>

                {{ $user->mrd_user_first_name }} {{ $user->mrd_user_last_name }}

            </td>

            <td>{{ $user->mrd_user_phone }}</td>
            <td>{{ $user->mrd_user_email }}</td>
